...
p_nrmse <- ggplot(metrics_model[metrics_model$metrics == "NRMSE", ], 
  ...
  stat_pvalue_manual(data = stat.test[stat.test$metrics == "NRMSE", ], 
                     label = "p.adj.signif", tip.length = 0.01, size = 8/.pt, hide.ns = FALSE) +
  ylab("NRMSE") + labs(tag = "C") +
  ...
  theme(panel.grid = element_blank(), strip.background.x = element_rect(fill = "white"), strip.placement = "outside", 
        ...
        plot.tag = element_text(size = 12, face = "bold"), plot.tag.position = "topleft")
...
p <- (p_feat_model / p_metrics) + plot_layout(heights = c(1.5, 4))
...
cairo_pdf(filename = "FigureS4.pdf", family = "Arial", width = 7.5, height = 5.5) 
p
dev.off()
