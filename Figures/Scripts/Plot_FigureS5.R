...
cor_df <- cor_df[!is.na(cor_df$value), ]
...
p_mfe <- ggplot(cor_df) +
  geom_tile(aes(x = Var1, y = Var2, fill = value, height = 0.9, width = 0.9), color = NA) +
  ...
  xlab("") + ylab("") +
  geom_text(aes(x = Var1, y = Var2, label = round(value, 2)), color = "black", size = 6/.pt) +
  scale_y_discrete(limits = rev(levels(cor_df$Var2))) +
  theme_bw(base_size = 7) + 
  theme(panel.grid = element_blank(), strip.background = element_rect(fill = "white"), strip.text = element_text(face = "italic", size = 7), 
        legend.position = "top", legend.key.height = unit(0.25, "cm"), legend.box.spacing = unit(0, "cm"),
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 6.5), axis.text.y = element_text(size = 6.5), 
        aspect.ratio = 1)
...
cor_combine$p_hw <- ifelse(test = cor_combine$p < 0.05, yes = 0.9, no = 0.6)
cor_combine$stop_ntp04 <- gsub("T", "U", cor_combine$stop_ntp04)
cor_combine$Sample <- recode_factor(cor_combine$Sample, Untr = "Untreated", Amik = "Amikacin", 
                                    G418_2000 = "G418 (2)", G418_500 = "G418 (0.5)", G418_500_10min = "G418 (0.5, 10 min)",  
                                    Genta = "Gentamicin", Neo = "Neomycin", Parom = "Paromomycin", Tobra = "Tobramycin")

p_stop4 <- ggplot(cor_combine, aes(x = stop_ntp04, y = Sample, fill = cor, height = p_hw, width = p_hw)) + 
  ...
  scale_y_discrete(limits = rev(levels(cor_combine$Sample))) +
  xlab("Stop codon and nt +4") + ylab("") +
  theme_bw(base_size = 7) + 
  theme(panel.grid = element_blank(), legend.position = "top", legend.title.align = 0.5, 
        axis.text.y = element_text(face = "italic", size = 6.5), axis.text.x = element_text(size = 6.5), axis.title.y = element_blank()) +
